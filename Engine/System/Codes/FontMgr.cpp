...
HRESULT Engine::CFontMgr::Ready_Font(LPDIRECT3DDEVICE9 pGraphicDev, const _tchar* pFontTag, const _tchar* pFontType, const _uint& iWidth, const _uint& iHeight, const _uint& iWeight)
{
	...
	NULL_CHECK_RETURN(pFontType, E_FAIL);
...
void Engine::CFontMgr::Render_Font(const _tchar* pFontTag, const _tchar* pString, const _vec2* pPostion, D3DXCOLOR Color)
{
	CFont*		pFont = Find_Font(pFontTag);
	NULL_CHECK(pFont);

	pFont->Render_Font(pString, pPostion, Color);
}
